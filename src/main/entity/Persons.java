...
public class Persons extends PriorityQueue<Person>{
	...
	public void save() {
		try {
			if (file.isFile() && file.exists() && file.canRead() && file.canWrite()) {
				...
				writer.flush();
				writer.close();
				System.out.println("Иноформация сохранена в фаил");
	 ...
	 */
	public void parse() throws RightException, SameIdException, FileNotFoundException {
			...
			while((line = reader.readLine()) != null) {
				if (!line.equals("[") && !line.equals("]") && !line.equals("")) {
					if (line.substring(line.length() - 1).equals(",")) {
						this.add(Person.parsePerson(line.substring(0,line.length()-1)));
					} else this.add(Person.parsePerson(line));
				}
			}
			fileReader.close();
			reader.close();
